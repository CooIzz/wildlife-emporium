# Wildlife Emporium

## Project Overview

Wildlife Emporium is a dynamic web application developed using HTML, CSS, JavaScript, PHP, and MySQL. The system is an interactive and dynamic learning platform about animals.

## Requirements

- WAMP Server
- A modern web browser

## Installation

1. Install and start WAMP Server.
2. Copy the `wildlife-emporium` project folder into the WAMP `www` directory.
3. Open phpMyAdmin through the WAMP server.

## Database Setup

1. Create a MySQL database named `wildlife_emporium` of type `utf8mb4_unicode_ci`.
2. Import the provided SQL database file from the SQL folder into the `wildlife_emporium` database.

## Running the Project

1. Open a web browser.
2. Access the project through the local WAMP server using its localhost address.
3. Register or log in to use the application's user-specific features.
4. Predefined user account has been created with the username: `user` and password: `password`
5. Predefined admin account has been created with the username: `admin` and password: `password`
